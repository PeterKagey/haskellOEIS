...
allPolyforms n k seedFacet = recurse k (Set.singleton seedFacet) (connectedFacets n seedFacet) where
  recurse :: Int -> Coloring -> Set Facet -> Set Coloring
...
addToPolyform :: Int -> Facet -> (Coloring, Set Facet) -> (Coloring, Set Facet)
...
rotation' :: Int -> Int -> Coloring -> Coloring
rotation' i j = Set.map $ Set.map (rotation i j)

flipFirst' :: Coloring -> Coloring
