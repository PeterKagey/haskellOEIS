...
a999998_list = (0,0) : recurse ((0,0), singleton (0,0), empty) where
...
updateStatus p_1 (p_0, points, slopes)
  | p_1 `member` points = Nothing
  | otherwise = if slope `member` slopes then Nothing else Just (p_1, points', slopes') where
    slope = slopeFrom p_0 p_1
    points' = p_1 `insert` points
    slopes' = slope `insert` slopes
